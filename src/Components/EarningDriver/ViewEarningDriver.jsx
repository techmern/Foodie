...
function ViewEarningDriver() {
  ...
  const [totals, setTotals] = useState({ totalCOD: 0 });
  ...
  const fetchTotals = async (fromDate = '', toDate = '') => {
    try {
      const response = await fetch(
        `http://localhost:5000/driverorder/countAmount?fromDate=${fromDate}&toDate=${toDate}&driverId=${loggedInDriver._id}`
      );
  ...
  return (
    <div>
      ...
      <div style={style.container}>
        <div style={style.maintitle}>View Today Earning</div>
        <div style={style.inputContainer}>
          <input
            ...
            title="Select a date to see the amount for that date"
          />
          <input
            type="date"
            value={toDate}
...
export default ViewEarningDriver;
