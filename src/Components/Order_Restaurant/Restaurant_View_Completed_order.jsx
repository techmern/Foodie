...
function Restaurant_View_Completed_order() {
        ...
        try {
            if (!loggedInRestaurant) {
                ...
                const filteredData = res.data.filter(order => {
                    const isStatusValid = order.deliveryStatus === 'Delivered';
                    const isOrderStatusValid = order.orderStatus === 'Ready';
                    return order.restaurant_id._id === loggedInRestaurant._id && isStatusValid && isOrderStatusValid;
                });
    ...
    const renderStars = (rating) => {
        ...
        if (rating === null) {
            for (let i = 0; i < 5; i++) {
                emptyStars.push(<span key={`empty-${i}`} style={style.starIconEmpty}>&#9734;</span>);
            }
            return emptyStars;
        }
        ...
        for (let i = 0; i < fullStars; i++) {
            emptyStars.push(<span key={`full-${i}`} style={style.starIcon}>&#9733;</span>);
        }
        if (halfStar) {
            emptyStars.push(<span key="half" style={style.starIcon}>&#9733;</span>);
        }
    ...
    return (
        <div>
            ...
            <div style={style.bodycolor}>
                <h1 style={style.testing}>View Completed Order</h1>
                <div style={style.searchInput}>
                    <span style={style.searchLabel}>From Date</span>
                    <input type="date" value={fromDate} onChange={handleFromDateChange} style={{ marginRight: '10px' }} />
                    <span style={style.searchLabel}>To Date</span>
                    <input type="date" value={toDate} onChange={handleToDateChange} style={{ marginRight: '10px' }} />
                </div>
                <div>
                    {resData.length === 0 ? (
                        <p style={{ textAlign: 'center', fontSize: '20px', marginTop: '20px' }}>No completed orders found.</p>
                    ) : (
                                ...
                                <div key={index} style={style.card}>
                                    ...
                                    <div style={style.cardBody}>
                                        ...
                                        <div><strong>User Address:</strong> {order.user_address}, {order.landmark}, {order.pincode}</div>  
                                         <br/>
                                        <div><strong>Driver Name:</strong> {order.drivername}</div>  
                                        <div><strong>Driver Mobile No:</strong> {order.phone_no}</div>
                                            ...
                                            {reviewId && (
                                                <>
                                                    ...
                                                    {selectedReview === reviewId && (
                                                        <div>
                                                            <textarea value={reply} onChange={handleReplyChange} placeholder="Write your reply here" rows="3" cols="50"/>
                                                            <br/>
                                                            <button style={style.btn} onClick={() => handleReplySubmit(reviewId)}>
                                                                Submit Reply
                                                            </button>
                                                        </div>
                                                    )}
...
export default Restaurant_View_Completed_order;
